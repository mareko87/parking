...
export class AddCenaSataToParkiraliste1612877284155 implements MigrationInterface {
    name = 'AddCenaSataToParkiraliste1612877284155'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `parkiraliste` ADD `cenaSata` int NOT NULL");
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `user` ADD CONSTRAINT `FK_4dd13cf5536c5ec906dba37cbef` FOREIGN KEY (`categoryId`) REFERENCES `user_category`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION");
        await queryRunner.query("ALTER TABLE `parkiraliste` DROP COLUMN `cenaSata`");
    }
